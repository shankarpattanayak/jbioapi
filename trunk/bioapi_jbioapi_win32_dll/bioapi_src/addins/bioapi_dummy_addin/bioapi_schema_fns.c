/*-----------------------------------------------------------------------
 * File: bioapi_schema_fns.c
 *-----------------------------------------------------------------------
 */


#include "precomp.h"
#include "bioapi_addin_schema.h"

#define _PRINTABLE_UUID_LENGTH	   ( 40 )
...
typedef struct bioapi_bsp_attr_state
{
	...
	uint8 PrintableUuid[_PRINTABLE_UUID_LENGTH];
	uint8 PrintableSpecVersion[_PRINTABLE_VERSION_LENGTH];
	uint8 PrintableProductVersion[_PRINTABLE_VERSION_LENGTH];
}
