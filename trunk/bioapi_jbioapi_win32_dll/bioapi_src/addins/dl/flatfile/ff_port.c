...
/* This is the size of every page in a file, PAGE_SIZE needs to be greator than or equal to
   ...
   number) */
#define PAGE_SIZE (s_dwAllocationGranularity)
...
#elif defined (UNIX)
		long page_size;
		if ((page_size = sysconf(_SC_PAGE_SIZE)) == -1L)
			return CSSMERR_DL_INTERNAL_ERROR;
		s_dwAllocationGranularity = page_size;
#else
#error Not implemented.
#endif
	}
...
#error Not implemented.
	...
	ASSERT(GetHeaderSize(pThis) + sizeof(struct ff_mmf_header) < PAGE_SIZE);
...
#elif defined (UNIX)
			uint64 pExtent = (uint64) GetExtentAddress(pThis);
			uint64 pPage = (pExtent / PAGE_SIZE) * PAGE_SIZE;
			DPRINTF("ff", "(ffport_mmf_eWrite) syncing page at 0x%p\n", pPage);
			if (msync((void *) pPage, PAGE_SIZE, MS_SYNC))
			{
				return MapErrno();
			}
#else
 ...
 * CSSM_OK if no error occurred, else CSSM_FAIL
 *---------------------------------------------------------------------------*/
FF_ERROR ffport_eGetPathStart(const char *DbName, char *szPathStart)
{
#if defined (WIN32)
	uint32 DirLen, DbLen;
	ASSERT(DbName);
	...
	/* First get the length of the current directory + 1 (for the NULL char) and the
	...
	else if (DirLen + DirLen < MAX_PATH_START_LENGTH)
	{
		...
		GetCurrentDirectory(MAX_PATH_START_SIZE, szPathStart);
		ASSERT(strlen(szPathStart) + 1 == DirLen);
	}
	else
	{
	...
	szPathStart[DirLen-1] = DIRECTORY_SEPARATOR;
	memcpy(&szPathStart[DirLen], DbName, DbLen);
	szPathStart[DirLen+DbLen] = 0;
	...
	if (DirLen + DbLen < MAX_PATH_START_LENGTH)
	{
		/* Get the current directory name */
		strcpy(szPathStart, cwd);
		ASSERT(strlen(szPathStart) + 1 == DirLen);
	}
	else
	{
		/* the current directory is too long, put it in /tmp */
		const char *tmpdir = "/tmp";
		strcpy(szPathStart, tmpdir);
		DirLen = strlen(tmpdir) + 1;
	}

	szPathStart[DirLen-1] = '/';
	memcpy(&szPathStart[DirLen], DbName, DbLen);
	szPathStart[DirLen+DbLen] = 0;
...
static void ffport_mmf_page_Deconstruct(FFPORT_MMF_PAGE *pThis)
{
	...
	if (pThis->pData != NULL)
	{
		int Result = munmap(pThis->pData, PAGE_SIZE);
		ASSERT(Result == 0);
	}
	...
	uint32 PageNumber)
{
	...
	if (PageNumber != pThis->PageNumber)
	{
		...
		ASSERT(PageNumber < (0xffffffff / PAGE_SIZE));
		pThis->hMapping = CreateFileMapping(
			pThis->hFile, /* hFile */
			&sa, /* pFileMappingAttributes */
			pThis->Mode, /* flProtect */
			0, /* dwMaximumSizeHigh */
			PAGE_SIZE * (PageNumber+1), /* dwMaximumSizeLow */
		...
		pThis->pData = (uint8 *)MapViewOfFile(
			...
			dwDesiredAccess, /* dwDesiredAccess */
			0, /* dwFileOffsetHigh */
			PageNumber * PAGE_SIZE, /* dwFileOffsetLow */
			PAGE_SIZE); /* dwNumberOfBytesToMap */
	...
	if (PageNumber != pThis->PageNumber)
	{
		off_t PageStart = PageNumber * PAGE_SIZE;
		off_t PageEnd	= PageStart	 + PAGE_SIZE;
		struct stat statbuf;
		int flags;
	...
	FFP_COPY_FUNC eCopyFunc)
{
	...
	ASSERT(Buffer);
	ASSERT(eCopyFunc);
	ASSERT(PAGE_SIZE && PAGE_SIZE >= s_dwAllocationGranularity);

	/* compute the last page we need an how much to write on that page, do not end by writing zero
	   bytes */
	EndPage = (StartOffset + Size - 1) / PAGE_SIZE;
	EndOffset = ((StartOffset + Size - 1) % PAGE_SIZE) + 1;

	StartPage = StartOffset / PAGE_SIZE;
	...
	for (cPage = StartPage; cPage <= EndPage; cPage++, StartOffset = 0)
	{
		...
		if (cPage == EndPage)
			...
			pageBytesToCopy = PAGE_SIZE - StartOffset;
...
static FF_ERROR ffp_eCopyToFile(uint8 *pFileData, void *pMemory, uint32 Size)
{
	...
	return FF_OK;
#elif defined (UNIX)
	uint64 pPage = ((uint64) pFileData / PAGE_SIZE) * PAGE_SIZE;
	...
	LOCK_STATUS *pLockStatus)
{
	...
	if (p_hLock)
	{
		...
		else
		{
			...
			uint32 LockStatus = *p_hLock - (uint32)pThis;
