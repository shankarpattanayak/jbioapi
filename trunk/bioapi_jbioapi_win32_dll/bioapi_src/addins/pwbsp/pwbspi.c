					...
					BioAPI_BIR_PTR		  *BIR )
{
	...
	if (pCurListNode->BIRHandle == BIRHandle)
	{
		*BIR = &pCurListNode->BIR;
		...
		/* TODO: SPI_RETURN(BioAPIERR_BSP_INVALID_BIR_HANDLE); */
		return (BioAPI_ERRCODE_INTERNAL_ERROR);
}
			...
			BioAPI_BIR_HANDLE_PTR	   ProcessedBIR )
{
	...
	if ((pBIRtoProcess->Header.Type & ~BioAPI_BIR_DATA_TYPE_SIGNED)
		!= (BioAPI_BIR_DATA_TYPE_RAW & ~BioAPI_BIR_DATA_TYPE_SIGNED)) {
		return (BioAPIERR_BSP_INVALID_BIR);
					...
					BioAPI_DATA_PTR *Payload )
{
	 ...
	 * and can be stripped */
	/*@@if ((BioAPI_BIR_DATA_TYPE_RAW & ~BioAPI_BIR_DATA_TYPE_SIGNED) !=
	...
	*/
	...
	if (BioAPI_PURPOSE_VERIFY != pCapturedBIR->Header.PurposeMask ||
		BioAPI_PURPOSE_VERIFY != pTemplateBIR->Header.PurposeMask)
...
{
	return BioAPI_ERRCODE_FUNCTION_NOT_IMPLEMENTED;
}
					...
					BioAPI_BIR_HANDLE_PTR AuditData )
{
	...
	/*------------------------------------------------------------------*
	 * Check for bad parameters and initialize output for the error case *
	 *------------------------------------------------------------------*/
