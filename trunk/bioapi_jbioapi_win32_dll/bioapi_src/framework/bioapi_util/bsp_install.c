...
{
	BioAPI_RETURN err = BioAPI_OK;
	CSSM_DB_SCHEMA_ATTRIBUTE_INFO_PTR DeviceAttrInfo = NULL;
	CSSM_DB_SCHEMA_INDEX_INFO_PTR DeviceIndexInfo = NULL;
	BioAPI_DEVICE_SCHEMA BioAPIBspDevice;
	uint32 format;

	if (INSTALL_ACTION_UNINSTALL != Action)
	{
		bioapi_Get_Bsp_Device_Schema_Attributes(&DeviceAttrInfo);
		bioapi_Get_Bsp_Device_Schema_Indexes(&DeviceIndexInfo);
		err = bioapi_CreateMDSRelation (MDS_CDSA_DIRECTORY,
	...
	if((BioAPI_OK == err || err == CSSMERR_DL_INVALID_RECORDTYPE) &&
		(INSTALL_ACTION_UNINSTALL != Action))
	{
		/* reset error to BioAPI_OK */
		err = BioAPI_OK;
		...
		bioapi_UpdateDeviceInfo( &BioAPIBspDevice, BioAPI_TRUE, device_uuid);
	}
