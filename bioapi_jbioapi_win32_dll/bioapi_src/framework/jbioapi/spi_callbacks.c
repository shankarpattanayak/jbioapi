...
NotifyFaultThread(void *Param)
{
	...
	if( threadDataPtr != NULL )
	{
		...
		ret = bioapi_BroadcastModuleEvent( &threadDataPtr->BSPUuid,
...
NotifyRemoveThread(void *Param)
{
	...
	if( threadDataPtr != NULL )
	{
		...
		retCode = bioapi_FindModuleByUUIDAndMultiLock( &threadDataPtr->BSPUuid,
												BioAPI_NO_LOCK,
												&pModuleEntry,
												BioAPI_WRITER_LOCK );
		if ( retCode != BioAPI_OK )
		...
		bioapi_BroadcastModuleEvent(&threadDataPtr->BSPUuid,
								  threadDataPtr->DeviceID,
								  threadDataPtr->EventType );
	}
	return 1;
