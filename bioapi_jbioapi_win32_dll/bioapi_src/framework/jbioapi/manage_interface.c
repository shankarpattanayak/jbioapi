...
BioAPI_UUID bioapi_UUID = ref_h_layer_uuid_init;
...
cssm_SWMRLock_PTR hModuleListSWMRLock = NULL; /* for syncronising the allocation and
deallocation of attached addinInfo structure */

void *hInitMutex = NULL;	/* This mutex is used serialize the BioAPI_Init's
							   and uninit's.
							*/
...
{
	BioAPI_RETURN err = BioAPI_OK;
	BioAPI_BOOL MutexCreated = BioAPI_FALSE;
	CSSM_DATA tmpPath;
	CSSM_DATA ModulePath;
	CSSM_DATA tmpModuleName;

	printf("Entra en BioAPI_Init (manage_interface)\n");
...
printf("kk1\n");
	...
	*/
	if(BioAPI_OK == err)
	{
		...
		if(NULL == hInitMutex)
		{
			ERR(err = port_CreateMutex(NULL,
									   &hInitMutex));
			if(BioAPI_OK == err)
			{
				MutexCreated = BioAPI_TRUE;

				ERR(err = port_LockMutex(hInitMutex,
										 CSSM_INFINITE_WAIT));
				if(BioAPI_OK != err)
				{
					ERR(err = BioAPIERR_H_FRAMEWORK_INTERNAL_ERROR);

					ERR(port_CloseMutex(hInitMutex));
					hInitMutex = 0;
				}
			}
		}
...
printf("kk8\n");
			...
			/* unlock the mutex and destroy it if necessary */
			ERR(port_UnlockMutex(hInitMutex));

			if((BioAPI_OK != err) &&
			   (BioAPI_TRUE == MutexCreated))
			{
				ERR(port_CloseMutex(hInitMutex));
			}
		}
	}
printf("Sale en BioAPI_Init (manage_interface)\n");
	return err;
}
