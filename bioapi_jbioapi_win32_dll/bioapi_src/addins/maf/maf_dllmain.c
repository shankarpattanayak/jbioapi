 ...
 * s_hUseLock.
 */
static void* s_hLoadLock = PORT_STATIC_MUTEX_INITIALIZER;
...
#define ATTACH_TRACKER_MAGIC_NUMBER( pTracker ) (~((uint32)(pTracker)))
...
#ifdef _DEBUG /* Debug version */
#define LOAD_TRACKER_MAGIC_NUMBER( pTracker ) ((uint32)(pTracker) - 5123)
...
static BioAPI_RETURN addin_InitResources()
{
	...
	/* Initialize the SP synch lock */
	if ( ( rv = cssm_SWMRLockCreate( &s_hUseLock,
									 NULL ) ) == BioAPI_OK )
	{
		...
		/* If something failed, delete the usage lock */
		if ( rv != BioAPI_OK )
		{
			cssm_SWMRLockDelete( &s_hUseLock );
		}
	}
...
#pragma warning(default: 4127)
			...
			else
			{
				...
				if ( port_CreateMutex( NULL, &s_hLoadLock ) != BioAPI_OK )
				{
					rv = FALSE;
				}
				else
	...
	case DLL_PROCESS_DETACH:
		{
			...
			if ( port_CloseMutex( s_hLoadLock ) != BioAPI_OK )
			{
				assert( 0 );
			}
