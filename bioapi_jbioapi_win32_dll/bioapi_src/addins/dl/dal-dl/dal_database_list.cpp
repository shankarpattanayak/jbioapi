...
functions that lock the mutex
	...
	DAL_DATABASE_INFO_LIST::DAL_DATABASE_INFO_NODE::GetDatabase(HANDLE &hMutex)
...
{
	CSSM_RETURN ret = port_CreateMutex(NULL, &m_hMutex);
	if (CSSM_OK != ret)
	{
...
CSSM_RETURN DAL_DATABASE_INFO_LIST::DAL_DATABASE_INFO_NODE::GetDatabase(HANDLE &hMutex,
 ...
 * hMutex (outpu) : Mutex that was locked in order to return the handle
 *
 * RETURN:
